...
function ProgrammeDesigner({ data, mutate }: { data: AppData; mutate: Mutate }) {
  ...
  const [phaseEditor, setPhaseEditor] = useState<{ id?: string; name: string; startDate: string; endDate: string; focus: string } | null>(null);
  ...
  return <><div className="view-stack programme-designer">
    ...
    {tab === "overview" ? <><section className="programme-overview-card"><div><p className="eyebrow">Current training block</p><h2>{block.name}</h2><p>{block.trainingGoal}</p><small>{formatDay(block.startDate, { weekday: undefined })} → {formatDay(block.endDate, { weekday: undefined })} · {data.weeks.length} programme weeks · {data.phases.length} phases</small></div><div className="dialog-actions"><Button variant="outline" onClick={() => setBlockEditor(true)}>Edit block</Button><Button variant="outline" onClick={() => mutate({ action: "viewDuoBase" }, "DUO base is the reference programme")}>View DUO base</Button></div></section><section><SectionHeading eyebrow="Phases" title="Editable programme phases" action={<Button size="sm" variant="outline" onClick={() => setPhaseEditor({ name: "", startDate: block.startDate, endDate: block.endDate, focus: "" })}>+ Add Phase</Button>} /><div className="programme-phase-list">{data.phases.map((phase, index) => <article key={phase.id}><span className="programme-phase-number">{index + 1}</span><div><strong>{phase.name}</strong><small>{formatDay(phase.startDate, { weekday: undefined })} → {formatDay(phase.endDate, { weekday: undefined })}</small><p>{phase.focus}</p></div><Button variant="outline" size="sm" onClick={() => setPhaseEditor(phase)}>Edit</Button></article>)}</div></section></> : null}
    ...
    {tab === "progressions" ? <section><SectionHeading eyebrow="Progression stimulus" title="Progression Tracks" /><p className="muted-panel">Tracks advance on completed planned sessions, not simply because a calendar week passed. Deloads remain Week Types.</p><div className="progression-track-list">{data.v2.progressionTracks.map((track) => <article className="progression-track-card" key={track.id}><div><h3>{track.name}</h3><p>{track.purpose}</p></div><ol>{track.steps.map((step, index) => <li className={index < track.currentStep ? "progression-step-complete" : index === track.currentStep ? "progression-step-current" : ""} key={step.id}><span>{index + 1}</span><div><strong>{step.title}</strong><small>{step.prescription}</small></div>{index === track.currentStep ? <small className="muted-copy">Advances after the planned session is completed.</small> : null}</li>)}</ol></article>)}</div><p className="empty-builder-state">No custom progression tracks yet? DUO base progressions are available above; custom tracks can be added without changing historical sessions.</p></section> : null}
  ...
  </div><Dialog open={blockEditor} onOpenChange={setBlockEditor}><DialogContent className="training-dialog" mode="responsive-editor"><DialogHeader><DialogTitle>Edit training block</DialogTitle><DialogDescription>Changing a block never deletes sessions, results or race events.</DialogDescription></DialogHeader><div className="dialog-form-grid"><div className="field-stack"><Label>Name</Label><Input value={blockDraft.name} onChange={(event) => setBlockDraft({ ...blockDraft, name: event.target.value })} /></div><div className="two-field-grid"><div className="field-stack"><Label>Start</Label><Input type="date" value={blockDraft.startDate} onChange={(event) => setBlockDraft({ ...blockDraft, startDate: event.target.value })} /></div><div className="field-stack"><Label>End</Label><Input type="date" value={blockDraft.endDate} onChange={(event) => setBlockDraft({ ...blockDraft, endDate: event.target.value })} /></div></div><div className="field-stack"><Label>Training goal</Label><Textarea value={blockDraft.trainingGoal} onChange={(event) => setBlockDraft({ ...blockDraft, trainingGoal: event.target.value })} /></div><div className="field-stack"><Label>Notes</Label><Textarea value={blockDraft.notes} onChange={(event) => setBlockDraft({ ...blockDraft, notes: event.target.value })} /></div></div><DialogFooter><Button variant="outline" onClick={() => setBlockEditor(false)}>Cancel</Button><Button onClick={async () => { await mutate({ action: "updateBlock", blockId: block.id, ...blockDraft }, "Training block saved"); setBlockEditor(false); }}>Save</Button></DialogFooter></DialogContent></Dialog><Dialog open={Boolean(phaseEditor)} onOpenChange={(open) => !open && setPhaseEditor(null)}><DialogContent className="training-dialog" mode="responsive-editor"><DialogHeader><DialogTitle>{phaseEditor?.id ? "Edit phase" : "Add phase"}</DialogTitle><DialogDescription>Phases cannot overlap and must remain inside this training block.</DialogDescription></DialogHeader>{phaseEditor ? <div className="dialog-form-grid"><div className="field-stack"><Label>Name</Label><Input value={phaseEditor.name} onChange={(event) => setPhaseEditor({ ...phaseEditor, name: event.target.value })} /></div><div className="two-field-grid"><div className="field-stack"><Label>Start</Label><Input type="date" value={phaseEditor.startDate} onChange={(event) => setPhaseEditor({ ...phaseEditor, startDate: event.target.value })} /></div><div className="field-stack"><Label>End</Label><Input type="date" value={phaseEditor.endDate} onChange={(event) => setPhaseEditor({ ...phaseEditor, endDate: event.target.value })} /></div></div><div className="field-stack"><Label>Focus</Label><Textarea value={phaseEditor.focus} onChange={(event) => setPhaseEditor({ ...phaseEditor, focus: event.target.value })} /></div></div> : null}<DialogFooter>{phaseEditor?.id ? <Button variant="destructive" onClick={async () => { await mutate({ action: "deletePhase", phaseId: phaseEditor.id }, "Phase removed"); setPhaseEditor(null); }}>Remove</Button> : null}<Button variant="outline" onClick={() => setPhaseEditor(null)}>Cancel</Button><Button onClick={async () => { if (!phaseEditor) return; await mutate({ action: phaseEditor.id ? "updatePhase" : "createPhase", phaseId: phaseEditor.id, blockId: block.id, ...phaseEditor }, "Phase saved"); setPhaseEditor(null); }}>Save</Button></DialogFooter></DialogContent></Dialog></>;
}
...
function WorkoutFormDialog({ item, data, open, onOpenChange, mutate, onSaved }: { item?: LibraryItem; data: AppData; open: boolean; onOpenChange: (value: boolean) => void; mutate: Mutate; onSaved: (id: string) => void }) {
  ...
  const [directHyroxStations, setDirectHyroxStations] = useState<string[]>(item ? data.v2.workoutCoverage[item.id]?.filter((coverage) => coverage.exposure === "direct").map((coverage) => coverage.station) ?? [] : []);
  ...
  const save = async () => {
    ...
    const result = await mutate({ action: item ? "updateWorkout" : "createWorkout", workoutId: item?.id, ...form, directHyroxStations, strengthTemplateId, prescription: form.mainSet || builderSlots.map((slot) => slot.prescription).join(" · ") }, item ? "Workout updated" : "Workout created");
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="training-dialog workout-form-dialog" mode="responsive-editor">
        ...
        <div className="dialog-form-grid">
          ...
          {form.family === "hyrox" ? <div className="field-stack"><Label>Direct HYROX areas <span className="optional-label">Optional</span></Label><div className="equipment-check-grid">{stations.map((station) => <Label key={station}><Checkbox checked={directHyroxStations.includes(station)} onCheckedChange={(checked) => setDirectHyroxStations((current) => checked ? [...current, station] : current.filter((item) => item !== station))} /><span>{station}</span></Label>)}</div></div> : null}
